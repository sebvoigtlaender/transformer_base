...
config.module_specs = {'actions': 'continuous', 'return': 'scalar'}

transformer = DecisionTransformer(config)

x = {'actions': pt.rand(config.batch_size, config.len_context, config.vocab_size),
'return': pt.rand(config.batch_size, config.len_context, 1), 
'local_position': pt.randint(0, config.len_context, (config.batch_size, config.len_context))}
